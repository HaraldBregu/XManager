...
class DashboardBloc extends Bloc<DashboardEvent, DashboardState> {
  ...
  DashboardBloc({
    ...
    on<GetTrainingProgramListEvent>(_onGetTrainingProgramsEvent);
    on<SelectTrainingProgramEvent>(_onSelectTrainingProgramEvent);
  }
  ...
  Future<void> _onGetTrainingProgramsEvent(
    GetTrainingProgramListEvent event,
    Emitter<DashboardState> emit,
  ) async {
    emit(const TrainingProgramsStateLoading());

    final response = await getProgramsUseCase.call(NoParams());
    response.fold(
      (left) {
        emit(const TrainingProgramStateError());
      },
      (right) {
        emit(TrainingProgramsStateSuccess(programs: right));
      },
    );
  }

  Future<void> _onSelectTrainingProgramEvent(
    SelectTrainingProgramEvent event,
    Emitter<DashboardState> emit,
  ) async {
    emit(
